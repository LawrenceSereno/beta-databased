...
function getPublicReviewEntries(limit = 8) {
  // Note: This function is called synchronously, so we need to use cached bookings
  // If you need real-time data, convert this to async
...
function renderCommunityReviewFeed(targetId = 'adminReviewFeed', limit = 6) {
  const container = document.getElementById(targetId);
  if (!container || typeof getPublicReviewEntries !== 'function') return;
  const entries = getPublicReviewEntries(limit);
  if (!entries.length) {
