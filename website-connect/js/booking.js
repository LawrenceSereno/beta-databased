...
async function getAvailableSlotsForTime(date, time) {
  ...
  for (const groomer of activeGroomers) {
    // Check if this groomer has capacity for this specific time slot
    if (typeof groomerSlotAvailable === 'function') {
      if (await groomerSlotAvailable(groomer.id, date, time)) {
        totalSlots += 1;
      }
    } else {
      // Fallback: check if groomer has any capacity for the day
      const limit = groomer.maxDailyBookings || (typeof GROOMER_DAILY_LIMIT !== 'undefined' ? GROOMER_DAILY_LIMIT : 3);
      ...
      // Each groomer can handle one booking per time slot
