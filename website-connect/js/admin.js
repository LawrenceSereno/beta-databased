...
function loadOverview() {
  ...
  const cancelledBookings = bookings.filter(b => ['cancelled', 'cancelledByCustomer', 'cancelledByAdmin'].includes(b.status)).length;
  ...
  updateStaffAlertPanel(absences);
  renderBlockedCustomersPanel();
  renderLiftBanPanel();
  renderCommunityReviewFeed('adminReviewFeed', 6);
}
...
function changeRecentBookingsLimit(limitValue) {
...
function blockSelectedDay() {
  ...
  customAlert.confirm(`Block ${formatDate(date)} for "${reason}"? All bookings on this day will be cancelled.`, (confirmed) => {
    if (!confirmed) return;
...
function unblockSelectedDay() {
  ...
  customAlert.confirm(`Re-open ${formatDate(date)} for bookings?`, (confirmed) => {
    if (!confirmed) return;
...
function cancelBookingsForDate(date, reason) {
  const bookings = getBookings();
  let changed = false;
...
function handleAddWarning(userId) {
  const user = getUsers().find(u => u.id === userId);
  if (!user) return;
  const reason = prompt(`Reason for warning for ${user.name}?`, 'No-show / late cancellation');
  if (reason === null) return;
  const info = incrementCustomerWarning(userId, reason.trim() || 'Admin issued warning');
...
function handleBanCustomer(userId) {
  const user = getUsers().find(u => u.id === userId);
  if (!user) return;
  const reason = prompt(`Why ban ${user.name}?`, 'Exceeded warning limit');
  if (reason === null) return;
  customAlert.confirm(`Ban ${user.name}? They will be blocked from booking until lifted.`, (confirmed) => {
    if (!confirmed) return;

    banCustomer(userId, reason.trim() || 'Admin manual ban');
...
function openBookingDetail(bookingId) {
  ...
  showModal(`
    <h3 style="margin-top:0;">${escapeHtml(booking.customerName)} · ${escapeHtml(booking.petName)}</h3>
...
function openGroomerAssignmentModal(bookingId) {
  const bookings = getBookings();
  const booking = bookings.find(b => b.id === bookingId);
  if (!booking) return;
...
function assignGroomerToBooking(bookingId, groomerId, groomerName) {
  const bookings = getBookings();
  const booking = bookings.find(b => b.id === bookingId);
  if (!booking) return;
...
function handleRescheduleSubmit(event, bookingId) {
  ...
  if ((newGroomerId !== booking.groomerId || newDate !== booking.date || newTime !== booking.time)) {
    ...
    if (conflictingBooking) {
      ...
      // Since customAlert.confirm is callback-based, we'll wrap the rest of the logic
      customAlert.confirm(`This time slot is already booked by ${conflictingBooking.customerName}. Cancel that booking and reschedule this one?`, (confirmed) => {
        if (confirmed) {
  ...
  function proceedWithReschedule() {
    ...
    const selectedGroomer = groomers.find(g => g.id === newGroomerId);
...
function handleMediaSubmit(bookingId) {
  const beforeInput = document.getElementById('beforeImageInput');
  const afterInput = document.getElementById('afterImageInput');
  const bookings = getBookings();
  const booking = bookings.find(b => b.id === bookingId);
  if (!booking) return;
  booking.beforeImage = beforeInput?.value?.trim() || '';
  booking.afterImage = afterInput?.value?.trim() || '';
  saveBookings(bookings);
  logBookingHistory({
    bookingId,
...
function handleMediaFileChange(event, inputId, previewId) {
  ...
  reader.onload = () => {
    ...
    updateMediaPreview(previewId, reader.result);
  };
...
function handleNoShowSubmit(bookingId) {
  const bookings = getBookings();
  const booking = bookings.find(b => b.id === bookingId);
  if (!booking) return;
  booking.status = 'cancelledByAdmin';
  ...
  saveBookings(bookings);
  const warningInfo = incrementCustomerWarning(booking.userId, `No-show on ${formatDate(booking.date)} at ${formatTime(booking.time)}`);
  logBookingHistory({
    bookingId,
    action: 'no_show',
...
function renderBlockedCustomersPanel() {
  const container = document.getElementById('blockedCustomersPanel');
  if (!container) return;
  const blocked = getUsers().filter(user => user.role === 'customer' && user.isBanned);
  const bookings = getBookings();
...
function openLiftBanModal(userId) {
  const users = getUsers();
...
function viewNoShowBookings(userId) {
  const bookings = getBookings();
  const user = getUsers().find(u => u.id === userId);
  const noShowBookings = bookings.filter(b =>
    b.userId === userId &&
...
function cancelBookingForDate(date, groomerId) {
  const bookings = getBookings();
  let cancelled = 0;
...
function handleLiftBan(userId, skipPrompt = false) {
  const users = getUsers();
  const user = users.find(u => u.id === userId);
  if (!user) return;
...
function submitGroomingNotes(bookingId) {
  ...
  if (booking) {
    ...
    logBookingHistory({
      bookingId,
      action: 'completed',
      message: `Grooming completed on ${formatDate(new Date().toISOString().split('T')[0])}. Service: ${notes || 'No details provided'}`,
      actor: 'admin'
    });
...
// Cancel booking
function cancelBooking(bookingId, note = '', skipPrompt = false) {
...
function loadWalkInReview() {
  const container = document.getElementById('walkInReviewContainer');
  if (!container) return;

  const packages = getPackages();
  const groomers = getGroomers();
  const selectedPackage = packages.find(p => p.id === walkInBookingData.packageId);
  const selectedGroomer = groomers.find(g => g.id === walkInBookingData.groomerId);
